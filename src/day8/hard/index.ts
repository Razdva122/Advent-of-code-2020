...
function main(): number {
  for (let i = 0; i < inputFormatted.length; i += 1) {
    const inputForModify = [...inputFormatted];
    if (inputForModify[i].operation === 'jmp') {
      inputForModify[i] = {
        operation: 'nop',
        value: inputForModify[i].value,
      };
    } else if (inputForModify[i].operation === 'nop') {
      inputForModify[i] = {
        operation: 'jmp',
        value: inputForModify[i].value,
      };
...
console.log(`Result of day8/hard -> ${main()}`);
